...
- Detached tests exercise configuration deltas, unknown fields, collection removals, typed legacy JSON, static job dictionaries, backups, damage target fallback, lane selection, action matching and stale recommendations.
...
6. Test Glow and Outline with hotbar paging, hidden hotbars, HUD scales, the main cross hotbar and both double cross hotbars. Check glow intensity, pulse, custom colors and nested frames for a shared support action. Check alignment in fullscreen/windowed mode and after moving the game between monitors.

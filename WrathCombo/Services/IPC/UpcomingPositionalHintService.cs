...
internal static class UpcomingPositionalHintService
{
    ...
    {
        if (Enhanced.RecommendationContext.Current is not null)
            return;
    ...
    internal static uint[]? GetWireSnapshot()
    {
        ...
        if (Enhanced.RecommendationContext.Current is null)
    ...
    internal static void Report(
        ...
        int gcdsUntil)
    {
        // Teaching evaluates several temporary targets; only live rotations publish hints.
        if (Enhanced.RecommendationContext.Current is not null)
            return;
